...
The background `_incident_followup_loop` (registered beside `_recurring_loop`, every `INCIDENT_FOLLOWUP_INTERVAL`) runs a pure `_incident_followup_tick`: it re-attempts a known fix for still-open `fixable` bot-errors (**dry-run-safe** — never presses buttons when `deliver=False`), sends `format_incident_followup` nags, and after `INCIDENT_GIVEUP_MINUTES` emits `format_incident_escalated` (honest wording: "stopping auto-retries" only if a retry actually ran, else "no automatic fix available") and stops. See [[Configuration#Incident lifecycle tracking]].

> [!important] Liveness routing + cross-source resolution (the SinFermera "silent 2h" fix)
> A 5-agent root-cause review found the lifecycle was *tracking* a class of problem it could neither fix nor close, so the owner got a flood of `🟠`/`⏳`/`❌` and never a `✅ Resolved`. Three structural fixes:
> - **Reroute, don't just alert.** The panel's own `⚠️…has not sent any messages…Please check it!` notice is a liveness signal, not a generic error — it now routes into the panel `/start` path (**R5** in [[Monitoring and Recovery Rules]]): alive ⇒ relaunch the stalled farm, dead ⇒ `🚨 needs PC`. Previously it fell through to a generic HIGH alert that never recovered.
> - **Resolution driven by health, all sources.** `✅ Resolved` is no longer reachable only via a `classify=="normal"` bot message on a `bot_error` incident (a recovered panel's status card classifies `unknown`). `incident_tracker.resolve_open_for_bot` closes **every** open incident for a panel (sources `bot_error`/`silence`/`panel`) the moment its `decide()` returns healthy, and emits exactly **one** closure (`_resolve_incidents_for`). This is what finally produces `✅ Resolved` for the "silent 2h" case.
> - **Channel coordination.** A fresh `bot_error` alert is suppressed while a `bot_error` incident is already open for that bot (lifecycle owns the cadence), and `_recurring_loop` skips bots with an open incident — so one problem no longer fans out across the `🟠` + `⏳` + `🔁` channels. Suppression is scoped to the **same source**, so a `silence`/`panel` incident can never swallow a genuinely new `bot_error`.
