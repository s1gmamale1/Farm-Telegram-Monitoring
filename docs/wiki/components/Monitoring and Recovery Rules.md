...
The panel's **own** watchdog notice — `[SinFermeraN] ⚠️Panel has not sent any messages for the last X hours Y minutes. Please check it!⚠️` — is a *self-reported* liveness failure (matched by `classifier.is_panel_silence_selfreport`). Crucially, that very notice is **fresh Telegram traffic**, so the age-based R6 clock never sees the chat as silent and never fires for it. WatcherDog therefore treats the notice itself as the probe trigger (**R5**) instead of letting it fall through to a generic error alert.
...
| **R5** | The panel's own `⚠️…has not sent any messages…Please check it!` watchdog notice (a *self-reported* silence; fresh traffic, so R6's age clock can't fire) | `/start`-probe → **alive**: relaunch (`Select 4/10 unfarmed` → `Start`, via the same `PANEL_AUTO_RECOVER` gate + confirm-card fallback as R2); **no reply**: `🚨 PC OFF` (as R6); **inconclusive**: retry next sweep | A first-class recovery **episode**: arms the same `recover_attempts` / `r2_attempted_ts` / `episode_issue` state as R2, so the **retry-cap** and the **R4** black-screen follow-up both apply to it. Debounced on the last *action* (`PANEL_ACTION_DEBOUNCE_SECONDS`), leaving R6's separate probe clock untouched. Handled in `_handle_panel_selfreport_silence`. |
...
**Retry-cap → cold case.** A frozen RDP host can't be fixed from Telegram, so the engine never loops forever: after `PANEL_MAX_ATTEMPTS` (default 3) failed Kill→Start cycles it stops acting and escalates the panel as **needs PC**, then stays quiet until it recovers. **R4** (black screenshot) and **R6** (silent + no `/start` reply) escalate immediately via the same one-line format; **R5** (self-reported silence) shares R2's episode counters, so its repeated failed relaunches hit the *same* retry-cap rather than nagging forever. The two cold cases have **different fixers**: an **R4** panel is *on* (it answered Telegram) but its render froze → the per-PC tool closes/reopens the frozen `wfreerdp` window and self-heals it; an **R6** panel is *off* → no software can touch it, a human must power it on.

**One closure message.** When a panel returns healthy, exactly ONE message is sent: the `… | Fixed ✅` episode line above when a recovery sequence ran (R2/R3/R5), or the canonical `✅ Resolved — … after Xm` from the [[Alerts and Heartbeat|incident lifecycle]] for a cold-case that had no action episode (R6 PC-off). The lifecycle's cross-source `resolve_open_for_bot` closes every open incident for the panel at once, so the old "back online" + "Resolved" double-message can't occur.
