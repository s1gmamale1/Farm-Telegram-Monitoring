...
asking a model to improvise. Account-level errors (ban / captcha / Steam
...
from watcherdog.classifier import needs_human
...
async def attempt(client, cfg, bot, text, *, chat=None, deliver=True):
    ...
    The human-needed gate runs FIRST — even on a dry run — because a ban /
    captcha / Steam Guard prompt is a human problem regardless of capabilities.
    (Only that account-level subset is exempt: crash/stuck/timeout novel errors
    are exactly what the restart ladder is FOR.)
    """
    if needs_human(text):
